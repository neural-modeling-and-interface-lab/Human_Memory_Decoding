...
for ca = categoryPool
    ...
    %% Model Evaluation - First layer predicting (using surrogate spikes)
    ...
    for currentFold = nestedFold
        ...
        for resolution = 1:length(MD_baselearner.resolution_all)
            ...
            for runTemp = 1:runTime
                ...
                % Here, for each neuron and trial we randomly shuffle
                % each spike using a jittering range
                jitterStep = 10; % 20 ms
                ...
                for trialIndex = 1:size(spikeTensor_train_0, 1)
                    for neuronIndex = 1:size(spikeTensor_train_0, 3)
                        ...
                        spikeIndex = find(tempContent==1);

                        tempContent_new = tempContent; tempContent_new(spikeIndex) = 0;

                        spikeIndex_new = zeros(size(spikeIndex));
                        for sIndex = 1:length(spikeIndex)
                            spikeIndex_new(sIndex) = spikeIndex(sIndex) + randi(jitterStep) * (-1)^randi(2);
                        end
                        spikeIndex_new(spikeIndex_new<=0) = []; spikeIndex_new(spikeIndex_new>size(spikeTensor_train_0, 2)) = [];
                        tempContent_new(spikeIndex_new) = 1;
                        spikeTensor_train_control_1(trialIndex, :, neuronIndex) = tempContent_new;
                    end
                end
                ...
                for trialIndex = 1:size(spikeTensor_test_0, 1)
                    for neuronIndex = 1:size(spikeTensor_test_0, 3)
                        ...
                        spikeIndex_new(spikeIndex_new<=0) = []; spikeIndex_new(spikeIndex_new>size(spikeTensor_train_0, 2)) = [];
                        tempContent_new(spikeIndex_new) = 1;
                        spikeTensor_test_control_1(trialIndex, :, neuronIndex) = tempContent_new;
                    end
                end
    ...
    oF = strcat('result\MD_baselearner_',thisCase(3:end), '_',Category,'_fold',mat2str(currentFold), '_control_2_outputs.mat');
    ...
    iF_2 = strcat('result\MD_baselearner_',thisCase(3:end), '_',Category,'_fold',mat2str(currentFold), '_control_2_outputs.mat');
